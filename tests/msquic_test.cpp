...
#define MSQUIC_ASIO_TEST_REQUIRE_EQUAL(x, y)                                   \
  boost::ut::expect(boost::ut::fatal(x == y));
...
int main() {
  ...
  "Basic"_test = [] {
    ...
    // not yet invoked
    MSQUIC_ASIO_TEST_REQUIRE_EQUAL(0, i);

    ioc.run();

    a.wait();

    MSQUIC_ASIO_TEST_REQUIRE_EQUAL(1, i);
    ...
    ev.async_wait([&i, starttime](const boost::system::error_code &ec) {
      ...
      std::chrono::milliseconds duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(endtime -
                                                                starttime);
      MSQUIC_ASIO_TEST_REQUIRE_EQUAL(duration.count() > 1000, true);
    });
    ...
    ioc.run();
    b.wait();
    MSQUIC_ASIO_TEST_REQUIRE_EQUAL(11, i);
  };
  ...
  "Oneshot"_test = [] {
    ...
    net::co_spawn(
        ...
        [rr = std::move(r)]() mutable -> net::awaitable<void> {
          ...
          MSQUIC_ASIO_TEST_REQUIRE_EQUAL(rr.get(), "hello");
        },
        net::detached);
