...
public:
  ...
  explicit posix_descriptor(const executor_type &ex): net::posix::basic_descriptor<executor_type>(ex){}

  template <typename WaitHandler>
  BOOST_ASIO_INITFN_RESULT_TYPE(WaitHandler,
      void (boost::system::error_code))
  async_wait(
      BOOST_ASIO_MOVE_ARG(WaitHandler) token)
  {
    ...
    return parent_type::async_wait(net::posix::descriptor_base::wait_type::wait_read, std::move(token));
  }
